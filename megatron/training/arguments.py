...
def parse_args(extra_args_provider=None, args_list=None, ignore_unknown_args=False):
    """Parse all arguments."""
    ...
    if ignore_unknown_args:
        args, _ = parser.parse_known_args(args=args_list)
    else:
...
def validate_args(args, defaults={}):
    ...
    if args.use_dist_ckpt_deprecated and args.rank == 0:
        print(
            "--use-dist-ckpt is deprecated and has no effect. Use --ckpt-format to select the checkpoint format."
        )
    if args.dist_ckpt_format_deprecated and args.rank == 0:
        print(
            "--dist-ckpt-format is deprecated and has no effect. Use --ckpt-format to select the checkpoint format."
        )
    ...
    if args.inference_dynamic_batching:
        ...
        assert args.inference_dynamic_batching_chunk_size % 256 == 0, (
...
def _add_inference_args(parser):
    ...
    group.add_argument(
        "--external-cuda-graph",
        action="store_true",
        help="Use CUDA graph capture and replay. The CUDA graphs aremanually captured in the training script.",
    )
    group.add_argument(
        "--cuda-graph-scope",
        type=str,
