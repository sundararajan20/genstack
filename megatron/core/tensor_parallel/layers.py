...
def _initialize_affine_weight_gpu(weight, init_method, partition_dim, stride=1, is_expert=False):
    ...
    # Handle non-CUDA case by falling back to CPU initialization
    if not torch.cuda.is_available():
        # Just use CPU initialization if CUDA is not available
        with torch.random.fork_rng():
            torch.manual_seed(1234 + partition_dim)  # Simple deterministic seed
            init_method(weight)
        return

    # For CUDA devices, use the CUDA RNG tracker
    if not is_expert:
...
class LinearWithGradAccumulationAndAsyncCommunication(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.gradient_accumulation_fusion = (
            gradient_accumulation_fusion if _grad_accum_fusion_available else False
        )
        ctx.allreduce_dgrad = allreduce_dgrad
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # <<< SR: Force fusion off if CUDA extension not available
